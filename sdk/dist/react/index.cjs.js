    ...
    checkout(payment) {
        ...
        if (paymentData.mode === 'redirect') {
            ...
            if (paymentData.reference)
                params.set('ref', paymentData.reference);
            if (paymentData.description)
                params.set('desc', paymentData.description);
            if (paymentData.customerEmail)
                params.set('email', paymentData.customerEmail);
            if (paymentData.metadata)
                params.set('meta', JSON.stringify(paymentData.metadata));
            if (paymentData.successUrl)
                params.set('success', paymentData.successUrl);
            if (paymentData.cancelUrl)
                params.set('cancel', paymentData.cancelUrl);
...
const AruviContext = react.createContext(null);
const AruviProvider = ({ config, children }) => {
    const [aruvi, setAruvi] = react.useState(null);
    ...
    const checkout = react.useCallback((payment) => {
        aruvi?.checkout(payment);
    }, [aruvi]);
    const closeCheckout = react.useCallback(() => {
        aruvi?.closeCheckout();
    }, [aruvi]);
    const createPaymentLink = react.useCallback((payment) => {
        return aruvi?.createPaymentLink(payment) || '';
    }, [aruvi]);
    const verifyPayment = react.useCallback(async (paymentId, expectedAmount) => {
    ...
    return jsxRuntime.jsx(AruviContext.Provider, { value: value, children: children });
};
// ============================================================================
// Hook
// ============================================================================
function useAruvi() {
    const context = react.useContext(AruviContext);
    if (!context) {
...
const AruviButton = ({ payment, onSuccess, onError, onCancel, label = 'Pay with Aruvi', variant = 'primary', size = 'medium', showLogo = true, disabled = false, className = '', style, }) => {
    ...
    return (jsxRuntime.jsx("button", { type: "button", className: `aruvi-button ${className}`, style: buttonStyle, onClick: handleClick, disabled: disabled || isLoading, children: isLoading ? (jsxRuntime.jsxs(jsxRuntime.Fragment, { children: [jsxRuntime.jsx("span", { style: {
                        width: '20px',
                        height: '20px',
                        border: '2px solid currentColor',
                        ...
                        borderRadius: '50%',
                        animation: 'aruvi-spin 1s linear infinite',
                    } }), jsxRuntime.jsx("span", { children: "Processing..." })] })) : (jsxRuntime.jsxs(jsxRuntime.Fragment, { children: [showLogo && (jsxRuntime.jsx("span", { dangerouslySetInnerHTML: { __html: ARUVI_LOGO_SVG }, style: { display: 'flex' } })), jsxRuntime.jsx("span", { children: label })] })) }));
};
const CheckoutTrigger = ({ payment, onSuccess, onError, onCancel, children, }) => {
    ...
    return jsxRuntime.jsx(jsxRuntime.Fragment, { children: children({ onClick: handleClick, isLoading }) });
};
const PaymentLink = ({ payment, children, className, style, newTab = false, }) => {
    ...
    return (jsxRuntime.jsx("a", { href: link, className: className, style: style, target: newTab ? '_blank' : undefined, rel: newTab ? 'noopener noreferrer' : undefined, children: children || `Pay $${payment.amount}` }));
};
...
exports.AruviButton = AruviButton;
exports.AruviProvider = AruviProvider;
exports.CheckoutTrigger = CheckoutTrigger;
exports.PaymentLink = PaymentLink;
exports.useAruvi = useAruvi;
